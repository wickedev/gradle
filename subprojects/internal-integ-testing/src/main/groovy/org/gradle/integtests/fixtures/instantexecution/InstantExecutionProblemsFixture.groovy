...
final class InstantExecutionProblemsFixture {
    ...
    private static ProblemsSummary extractSummary(String text) {
        def headerPattern = Pattern.compile("(\\d+) instant execution (problems were|problem was) found(, (\\d+) of which seem unique)?.*")
...
""")
        ...
        for (int i = 0; i < uniqueProblems; i++) {
            ...
            if (!summary.first.matches(docPattern)) {
                throw new AssertionFailedError("""Expected a problem documentation link, found: ${summary.first}""")
            }
            summary = summary.drop(1)
        }

        return new ProblemsSummary(totalProblems, uniqueProblems, messages)
    }
